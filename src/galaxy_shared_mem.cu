...
__global__ void FillHistogram(unsigned int *histogram, int NoofReal, float *ra1, float *decl1, int NoofSim, float *ra2, float *decl2)
{
    ...
    __shared__ float shared_ra2[threadsperblock];
    __shared__ float shared_decl2[threadsperblock];
    __shared__ unsigned int shared_histogram[totalBins];

    if (threadIdx.x < totalBins)
    {
    ...
    {
        float cosDecl1 = cos(decl1[threadId]);
        float sinDecl1 = sin(decl1[threadId]);
        float degree = 0.0;
        ...
        for (int i = 0; i < NoofReal; i += threadsperblock)
        {
            ...
            shared_ra2[threadIdx.x] = ra2[i + threadIdx.x];
            shared_decl2[threadIdx.x] = decl2[i + threadIdx.x];

            __syncthreads(); // Ensure all threads have finished with shared memory

            for (int j = 0; j < min(threadsperblock, NoofReal - i); j++)
            {
                float cosAngle = cosDecl1 * cosf(shared_decl2[j]) * cosf(ra1[threadId] - shared_ra2[j]) + sinDecl1 * sinf(shared_decl2[j]);
